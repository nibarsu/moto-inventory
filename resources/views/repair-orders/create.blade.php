        ...
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('repair-orders.store') }}" class="space-y-6">
                        @csrf

                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                <x-input-label for="wo_no" value="工單號碼" />
                                <x-text-input id="wo_no" name="wo_no" type="text" class="mt-1 block w-full" :value="old('wo_no', $defaultWorkOrderNo)" required />
                        ...
                        <div>
                            ...
                            <textarea id="complaint" name="complaint" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('complaint') }}</textarea>
                            <x-input-error :messages="$errors->get('complaint')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="diagnosis" value="檢查結果 / 處理內容" />
                            <textarea id="diagnosis" name="diagnosis" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('diagnosis') }}</textarea>
