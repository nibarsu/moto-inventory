    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('parts.update', $part) }}" class="space-y-6 p-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="part_no" name="part_no" type="text" class="mt-1 block w-full" :value="old('part_no', $part->part_no)" required autofocus />
                            <x-input-error class="mt-2" :messages="$errors->get('part_no')" />
                        </div>

                        <div>
                            <x-input-label for="barcode" value="條碼" />
                            <x-text-input id="barcode" name="barcode" type="text" class="mt-1 block w-full" :value="old('barcode', $part->barcode)" />
                        ...
                        <div>
                            ...
                            <select id="brand_id" name="brand_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach ($brands as $brand)
                                    <option value="{{ $brand->id }}" @selected((string) old('brand_id', $part->brand_id) === (string) $brand->id)>{{ $brand->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid gap-6 sm:grid-cols-4">
                        <div>
                            ...
                            <x-text-input id="unit" name="unit" type="text" class="mt-1 block w-full" :value="old('unit', $part->unit)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('unit')" />
                        </div>

                        <div>
                            <x-input-label for="last_cost_price" value="最近成本" />
                            <x-text-input id="last_cost_price" name="last_cost_price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('last_cost_price', $part->last_cost_price)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('last_cost_price')" />
                        </div>

                        <div>
                            <x-input-label for="sale_price" value="售價" />
                            <x-text-input id="sale_price" name="sale_price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('sale_price', $part->sale_price)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('sale_price')" />
                        </div>

                        <div>
                            <x-input-label for="safety_stock" value="安全庫存" />
                            <x-text-input id="safety_stock" name="safety_stock" type="number" min="0" class="mt-1 block w-full" :value="old('safety_stock', $part->safety_stock)" required />
                    ...
                    <div>
                        ...
                        <textarea id="remark" name="remark" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('remark', $part->remark) }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('remark')" />
                    </div>

                    <div>
                        <label for="is_active" class="inline-flex items-center">
                            <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" @checked(old('is_active', $part->is_active))>
