    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500">{{ $purchaseOrder->po_no }}</p>
            </div>
            <a href="{{ route('purchase-orders.items.create', $purchaseOrder) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                新增明細
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @forelse ($items as $item)
                                    <tr>
                                        ...
                                        <td class="whitespace-nowrap px-4 py-3 text-right text-sm font-medium">
                                            <a href="{{ route('purchase-orders.items.edit', [$purchaseOrder, $item]) }}" class="text-indigo-600 hover:text-indigo-900">編輯</a>
                                            <form method="POST" action="{{ route('purchase-orders.items.destroy', [$purchaseOrder, $item]) }}" class="ms-4 inline" onsubmit="return confirm('確定要刪除此進貨單明細嗎？');">
                                                @csrf
                                                @method('DELETE')
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-4 py-8 text-center text-sm text-gray-500">目前沒有進貨單明細資料。</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="mt-6 flex items-center justify-between">
                        <a href="{{ route('purchase-orders.show', $purchaseOrder) }}" class="text-sm text-gray-600 hover:text-gray-900">返回進貨單</a>
                        <div class="text-sm font-semibold text-gray-900">
                            總金額：{{ number_format($purchaseOrder->fresh()->total_amount, 2) }}
                        </div>
                    </div>
