...
<body>
    @php
        $rows = $salesShipment->items->values();
    ...
    <div class="sheet">
        ...
        <div class="subhead">
            <table class="meta-table">
                <tr>
                    ...
                    <td>{{ $salesShipment->customer?->name ?: '-' }}</td>
                    <td class="label">倉庫</td>
                    <td>{{ $salesShipment->warehouse?->name ?: '-' }}</td>
                </tr>
                <tr>
            ...
            <table class="meta-table">
                <tr>
                    ...
                    <td>{{ $salesShipment->shipment_no }}</td>
                </tr>
                <tr>
                    <td class="label">日期</td>
                    <td>{{ optional($salesShipment->shipment_date)->format('Y-m-d') }}</td>
                </tr>
                <tr>
                    <td class="label">建立者</td>
                    <td>{{ $salesShipment->creator?->name ?: '-' }}</td>
                </tr>
            </table>
        </div>
                ...
                <tbody>
                    @for ($i = 0; $i < $minRows; $i++)
                        ...
                        <tr>
                            ...
                            <td class="right">{{ $item ? number_format((float) $item->unit_price, 2) : '' }}</td>
        ...
        <div class="notes-row">
            <div class="notes-box">
                <div>附記：</div>
                <div>1. 本單據作為出貨與對帳依據。</div>
                <div>2. 請收貨後核對商品與數量，如有差異請即時通知。</div>
            </div>

            <table class="summary-table">
                <tr>
                    <td class="label">小計</td>
                    <td class="amount">{{ number_format((float) $salesShipment->total_amount, 2) }}</td>
                </tr>
                <tr>
        ...
        <div class="signatures">
            <div class="line">客戶簽收</div>
            <div class="line">出貨人簽章</div>
