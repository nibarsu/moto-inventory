<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <h2 class="text-xl font-semibold leading-tight text-gray-800">快速進貨單</h2>
            <div class="flex gap-2">
                <a href="{{ route('quick-purchases.print', $purchaseReceipt) }}" target="_blank" class="inline-flex items-center rounded-md bg-gray-800 px-4 py-2 text-sm font-semibold text-white hover:bg-gray-700">列印 A4</a>
                <a href="{{ route('quick-purchases.create') }}" class="inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">再建一張</a>
            </div>
        </div>
    </x-slot>
            ...
            <div class="rounded-lg border border-gray-200 bg-white shadow-sm">
                <div class="border-b border-gray-200 px-6 py-5">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            <div class="text-sm tracking-[0.2em] text-gray-500">PURCHASE DOCUMENT</div>
                            <div class="mt-1 text-3xl font-bold text-gray-900">進貨單</div>
                        </div>
                        <div class="grid gap-3 sm:grid-cols-3">
                            <div class="rounded-md bg-gray-50 px-4 py-3">
                                <div class="text-xs text-gray-500">單號</div>
                                <div class="mt-1 font-semibold text-gray-900">{{ $purchaseReceipt->receipt_no }}</div>
                            </div>
                            <div class="rounded-md bg-gray-50 px-4 py-3">
                                <div class="text-xs text-gray-500">日期</div>
                                <div class="mt-1 font-semibold text-gray-900">{{ optional($purchaseReceipt->receipt_date)->format('Y-m-d') }}</div>
                            </div>
                            <div class="rounded-md bg-gray-50 px-4 py-3">
                                <div class="text-xs text-gray-500">建立者</div>
                                <div class="mt-1 font-semibold text-gray-900">{{ $purchaseReceipt->creator?->name ?: '-' }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid gap-6 px-6 py-5 lg:grid-cols-[1.5fr_1fr]">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="rounded-md border border-dashed border-gray-300 px-4 py-3">
                            <div class="text-xs text-gray-500">供應商</div>
                            <div class="mt-1 text-base font-semibold text-gray-900">{{ $purchaseReceipt->supplier?->name ?: '-' }}</div>
                        </div>
                        <div class="rounded-md border border-dashed border-gray-300 px-4 py-3">
                            <div class="text-xs text-gray-500">倉庫</div>
                            <div class="mt-1 text-base font-semibold text-gray-900">{{ $purchaseReceipt->warehouse?->name ?: '-' }}</div>
                        </div>
                    </div>
                    <div class="rounded-md border border-dashed border-gray-300 px-4 py-3">
                        <div class="text-xs text-gray-500">備註</div>
                        <div class="mt-1 min-h-[48px] text-sm leading-6 text-gray-700">{{ $purchaseReceipt->remark ?: ' ' }}</div>
                    </div>
                </div>
                    ...
                    <div class="mt-5 flex justify-end">
                        <div class="w-full max-w-sm rounded-md border border-gray-200 bg-gray-50">
                            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 text-sm">
                                ...
                                <span class="font-semibold text-gray-900">{{ number_format((float) $purchaseReceipt->total_amount, 2) }}</span>
                            </div>
                            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 text-sm">
                            ...
                            <div class="flex items-center justify-between px-4 py-4 text-base">
                                ...
                                <span class="text-xl font-bold text-gray-900">{{ number_format((float) $purchaseReceipt->total_amount, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
