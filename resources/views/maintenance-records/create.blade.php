        ...
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('maintenance-records.store') }}" class="space-y-6">
                        @csrf

                        <div class="grid gap-6 md:grid-cols-2">
                            <div>
                                <x-input-label for="record_no" value="紀錄編號" />
                                <x-text-input id="record_no" name="record_no" type="text" class="mt-1 block w-full" :value="old('record_no', $defaultRecordNo)" required />
                                <x-input-error :messages="$errors->get('record_no')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="service_date" value="保養日期" />
                                <x-text-input id="service_date" name="service_date" type="date" class="mt-1 block w-full" :value="old('service_date', now()->toDateString())" required />
                            ...
                            <div>
                                ...
                                <select id="repair_order_id" name="repair_order_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    ...
                                    @foreach ($repairOrders as $repairOrder)
                                        <option value="{{ $repairOrder->id }}" @selected(old('repair_order_id') == $repairOrder->id)>{{ $repairOrder->wo_no }}{{ $repairOrder->customer ? ' / '.$repairOrder->customer->name : '' }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="service_type" name="service_type" type="text" class="mt-1 block w-full" :value="old('service_type')" required />
                                <x-input-error :messages="$errors->get('service_type')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="plate_no" value="車牌號碼" />
                                <x-text-input id="plate_no" name="plate_no" type="text" class="mt-1 block w-full" :value="old('plate_no')" />
                                <x-input-error :messages="$errors->get('plate_no')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="mileage" value="目前里程" />
                                <x-text-input id="mileage" name="mileage" type="number" min="0" class="mt-1 block w-full" :value="old('mileage')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="next_service_mileage" name="next_service_mileage" type="number" min="0" class="mt-1 block w-full" :value="old('next_service_mileage')" />
                        ...
                        <div>
                            ...
                            <textarea id="service_content" name="service_content" rows="4" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('service_content') }}</textarea>
